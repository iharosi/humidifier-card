...
`;let Pt=class extends rt{constructor(){super(...arguments),this._entities={},this._pending={},this._pendingTimers=new Map,this._warnedMissing=!1,this._modeSelected=t=>{const e=this._entities.mode,i=this._stateObj("mode");if(!e||!i||!this.hass)return;const s=t.target.value;s&&s!==i.state&&(this._setPending("mode",s),this.hass.callService("select","select_option",{entity_id:e,option:s}))},this._fanInput=t=>{const e=Number(t.target.value);Number.isFinite(e)&&this._setPending("fan_level",e)},this._fanChange=t=>{const e=this._entities.fan_level;if(!e||!this.hass)return;const i=Number(t.target.value);Number.isFinite(i)&&(this._setPending("fan_level",i),this.hass.callService("number","set_value",{entity_id:e,value:i}))}}static getConfigElement(){return document.createElement(gt)}static getStubConfig(){return{prefix:"smart_humidifier"}}setConfig(t){!function(t){if(!t||"object"!=typeof t)throw new Error("humidifier-card: no configuration provided");if(t.hide&&!Array.isArray(t.hide))throw new Error("humidifier-card: `hide` must be a list of slot names");const e=(t.hide??[]).filter(t=>!$t.includes(t));if(e.length)throw new Error(`humidifier-card: unknown slot(s) in \`hide\`: ${e.join(", ")}. Valid slots: ${$t.join(", ")}`);const i=Object.keys(t.entities??{}).filter(t=>!$t.includes(t));if(i.length)throw new Error(`humidifier-card: unknown slot(s) in \`entities\`: ${i.join(", ")}. Valid slots: ${$t.join(", ")}`);if(!t.prefix&&!t.entities?.power)throw new Error("humidifier-card: set `prefix` (e.g. smart_humidifier) or at least `entities.power`")}(t),this._config={show_status:!0,dim_when_off:!1,...t},this._entities=function(t){const e=new Set(t.hide??[]),i={};for(const s of $t){if(e.has(s))continue;const n=t.entities?.[s];if(n)i[s]=n;else if(t.prefix){const{domain:e,suffix:n}=bt[s];i[s]=`${e}.${t.prefix}${n}`}}return i}(this._config),this._warnedMissing=!1,this._clearAllPending()}getCardSize(){return 2}disconnectedCallback(){super.disconnectedCallback(),this._clearAllPending()}shouldUpdate(t){if(!this._config)return!1;if(t.size>1||!t.has("hass"))return!0;const e=t.get("hass");return!e||!this.hass||Object.values(this._entities).some(t=>e.states[t]!==this.hass.states[t])}updated(t){super.updated(t),this._dropSettledPending(),this._warnAboutMissingEntities()}render(){if(!this._config||!this.hass)return q;const t=this._stateObj("power");if(!t)return B`<ha-card>
    ...
    </span>`}_renderToggle(t){const e=this._stateObj(t);if(!e)return q;const i=this._isAvailable(e),s="on"===(this._pending[t]??e.state),n=yt[t];return B`<button
    ...
    </button>`}_renderMode(t){const e=this._stateObj("mode");if(!e)return q;const i=this._isAvailable(e),s=e.attributes.options??[],n=this._pending.mode??e.state;return B`<ha-select
      naturalMenuWidth
      fixedMenuPosition
      aria-label=${vt.mode}
    ...
    >
      ${s.map(t=>B`<ha-list-item .value=${t}>${this._format(e,t)}</ha-list-item>`)}
    </ha-select>`}_renderFan(t){const e=this._stateObj("fan_level");if(!e)return q;const i=this._isAvailable(e),s=Number(e.attributes.min??1),n=Number(e.attributes.max??3),o=Number(e.attributes.step??1),r=Number(this._pending.fan_level??e.state),a=Number.isFinite(r)?r:s;return B`<div class="slider-wrap">
      <input
        type="range"
        min=${s}
        max=${n}
